################################# Attribution utils.py variant for LIG onto each concept ########
...
def eval_attributions(model, tokenizer, train_inputs, train_masks, y_train, layer, mode='normal', n_class = 5, concept_number = None):
    ...
    for i in range(train_inputs.shape[0]):
        ...
        for j in range(nloops):
            ...
            attribution, delta = lig.attribute(
                inputs=(train_inputs[i:i+1],train_masks[i:i+1]), 
                target=target, 
                return_convergence_delta=True,
                n_steps=50,
                additional_forward_args = (concept_number,) )
...
def get_attributions(model, tokenizer, train_inputs, train_masks, y_train, layer, mode='normal', concept_number = None):

    outputs, predictions, attributions, attributions_sum, attributions_target, deltas, tokens, texts = eval_attributions(model, tokenizer, train_inputs, train_masks, y_train, layer=layer, mode=mode, concept_number=concept_number )

    return outputs, predictions, attributions, attributions_sum, attributions_target, deltas, tokens, texts

def display_attributions(model, tokenizer, train_inputs, train_masks, y_train, layer, mode='normal', per_token=True, concept_number = None):
...
def process_batch(residual_model, tokenizer, batch_inputs, batch_masks, batch_targets, concept_number = None):
    outputs_, predictions_, attributions_, attributions_sum_, attributions_target_, deltas_, tokens_, texts_ = get_attributions(
        residual_model.forward_per_concept,
        tokenizer,
        batch_inputs, 
        batch_masks, 
        batch_targets, 
...
def process_data_in_batches(residual_model, tokenizer, inputs, masks, targets, batch_size = 1, example_index=None, concept_number = None):
    ...
    for batch_inputs, batch_masks, batch_targets in tqdm(batch_generator(inputs, masks, targets, batch_size), 
        ...
        outputs_, predictions_, attributions_, attributions_sum_,attributions_target_, deltas_, tokens_, texts_ = process_batch(residual_model, tokenizer, batch_inputs, batch_masks, batch_targets, concept_number)
...
def example_attribution(residual_model, tokenizer, input_tensor, mask_tensor, target_tensor, concept_number = None):
    ...
    outputs_, predictions_, attributions_, attributions_sum_, attributions_target_, deltas_, tokens_, texts_ = process_batch(
        residual_model, tokenizer, input_tensor.unsqueeze(0), mask_tensor.unsqueeze(0), target_tensor.unsqueeze(0), concept_number
    )
