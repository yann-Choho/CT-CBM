...
def compute_cavs_mean_minus(df_aug, embedder_model, embedder_tokenizer, baseline_model, config):
    """
    Crée les CAVs des macro-concepts en utilisant la méthode mean_minus_others
    sans passer par un DataLoader.

    Args:
        df_aug (pd.DataFrame): DataFrame augmentée contenant au moins les colonnes 'text', 'label'
                               et des colonnes 'dummy_*'.
        embedder_model (torch.nn.Module): Modèle d'embeddings.
        embedder_tokenizer: Tokenizer associé au modèle.
        baseline_model (torch.nn.Module): Modèle baseline fournissant la méthode get_output_cavs.
        config (object): Objet de configuration avec les attributs nécessaires (device, max_len, SAVE_PATH, etc.)

    Returns:
        dict: Dictionnaire des CAVs calculés pour chaque macro-concept.
    """
    import os
    import json
    import torch
    ...
    # Calcul des embeddings en itérant sur chaque ligne du DataFrame
    for idx, row in tqdm(train_data_saved.iterrows(), total=len(train_data_saved), unit="exemple"):
        text = row['text']
        # Tokenisation du texte (ajustez les paramètres selon vos besoins)
        ...
        # Déplacer les tenseurs sur le device (GPU ou CPU)
        ...
        # Stocker l'embedding sur CPU pour libérer la mémoire GPU
        embedding_cpu = outputs.flatten().detach().cpu()
        train_data_saved.at[idx, 'embeddings'] = embedding_cpu
        
        # Optionnel : libération de la mémoire GPU
        del input_ids, attention_mask, outputs
        torch.cuda.empty_cache()
    
    # Calcul des CAVs pour chaque concept
    cavs_concepts = {}
    for concept in concept_name_list:
        # Sélection des embeddings correspondant au concept
        ...
        if concept_data.empty:
            ...
            continue
        concept_embeddings = torch.stack([emb for emb in concept_data['embeddings'].values]).to(device)
        
        # Sélection des embeddings ne correspondant pas au concept
        ...
        if other_data.empty:
            ...
            continue
        other_embeddings = torch.stack([torch.tensor(emb) for emb in other_data['embeddings'].values]).to(device)
        
        # Calcul du CAV : moyenne des embeddings du concept moins moyenne des autres embeddings
        cav = concept_embeddings.mean(dim=0) - other_embeddings.mean(dim=0)
        cavs_concepts[concept] = cav.cpu()  # Conserver sur CPU
        
    # Sauvegarde des CAVs dans un fichier JSON
